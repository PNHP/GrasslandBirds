...
excelfile <- "bird_veg_1409_Grasslands_final_ModifiedVersion.xlsx"
...
SiteAttributes <- read_excel(here(filedirectory,excelfile), sheet="SiteAttributes")
SiteVeg <- read_excel(here(filedirectory,excelfile), sheet="Vegetation")
SurveyVisit <- read_excel(here(filedirectory,excelfile), sheet="SurveyVisit")
...
SurveyVisit$Jdate <- as.POSIXlt(SurveyVisit$date_start, format="%m-%d-%Y")$yday+1

# join lat/lon data from SiteAttributes
rawdat <- merge(SurveyVisit,SiteAttributes[c("pt_id","X","Y")])
...
rawdat$X <- NULL # can rejoin from SiteAttributes if needed
rawdat$Y <- NULL # can rejoin from SiteAttributes if needed
rawdat$time_start <- NULL # not used again after decimal start time is calculated
rawdat$date_start <- NULL # not used again
rawdat$starttime <- NULL # not used again
rawdat$Sunrise <- NULL # not used again
...
# build the unique_id_variable
rawdat$unique_id <- paste(rawdat$pt_id,rawdat$survey_yr,rawdat$survey_vis,sep="_")
...
###speclist <- unique(rawdat$elem_name)  ## NOTE that no real error checking is being done on this list
...
rawdat2$spec_pt <- paste(rawdat2$elem_name, rawdat2$pt_id, sep="_") # add field with unique combo of species and point
...
spec_pt_list <- unique(temprawdat$spec_pt)

uniqueIDlist <- vector()
for (i in 1:length(spec_pt_list)){
  temp <- subset(temprawdat, temprawdat$spec_pt==spec_pt_list[i])
...
for (s in speclist){
  ...
  zeropts <- rbind(zeropts, data.frame(pt_id=misspt, elem_name=rep(s, length(misspt)), tot_sing=rep(0, length(misspt)), distance=rep(0, length(misspt)), sing1=rep(0, length(misspt)), sing2=rep(0, length(misspt)), sing3=rep(0, length(misspt)),sing4=rep(0, length(misspt)),sing5=rep(0, length(misspt)),spec_pt=paste(s,misspt,sep="_"))) ## DY- added Sing1...    sing5=rep(0, length(misspt)), 
}


#drop everything from rawdatsv in order to rbind it to zeropts
rawdatsv_tmp <- rawdatsv[c(colnames(zeropts))]
rawdatsv_tmp <- rbind(rawdatsv_tmp,zeropts)

#colremove <- names(rawdatsv_tmp)
#colremove <- colremove[!(colremove %in% c("pt_id"))] # ,"spec_pt"

#rawdatsv_joinfields <- rawdatsv[!(colnames(rawdatsv) %in% colremove)]
rawdatsv_joinfields <- rawdatsv[c("pt_id","elem_name","nosing1","nosing2","nosing3","nosing4","nosing5")]

zerodat <- merge(rawdatsv_tmp, rawdatsv_joinfields, by.x=c("pt_id","elem_name"), by.y=c("pt_id","elem_name"), all.x=TRUE, all.y=FALSE)
zerodat <- zerodat[c("pt_id","elem_name","spec_pt","distance","tot_sing","sing1","sing2","sing3","sing4","sing5","nosing1","nosing2","nosing3", "nosing4","nosing5")]
# fill in zeros instead of NAs
zerodat[is.na(zerodat)] <- 0

rawdatsv_join <- rawdatsv[c("pt_id",setdiff(names(rawdatsv),names(zerodat)))]
rawdatsv_join$uniqueID <- NULL
rawdatsv_join$TotSing <- NULL
rawdatsv_join$tot_all <- NULL
rawdatsv_join$All_detections <- NULL
#rawdatsv_join$survey_yr
#rawdatsv_join$survey_vis


# merge with rawdat to fill in other columns
zerodat1 <- merge(unique(zerodat), unique(rawdatsv[c("pt_id",)]), by.x=c("pt_id"), by.y=c("pt_id"),all.x=TRUE, all.y=FALSE)
...
ptctTot <- cleandat[!(duplicated(cleandat$spec_pt)),]
